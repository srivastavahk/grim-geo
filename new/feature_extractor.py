...
class FeatureExtractor:
    def __init__(self):
        ...
        self.dino_model = AutoModel.from_pretrained(GRIMConfig.DINO_MODEL).to(GRIMConfig.DEVICE)

        # Text Model: CLIP
        self.clip_model = CLIPModel.from_pretrained(GRIMConfig.CLIP_MODEL).to(GRIMConfig.DEVICE)
    ...
    def extract_task_embedding(self, task_description: str) -> torch.Tensor:
        ...
        """
        inputs = self.clip_tokenizer([task_description], padding=True, return_tensors="pt").to(GRIMConfig.DEVICE)
        with torch.no_grad():
    ...
    def extract_dino_features(self, image: Image.Image):
        ...
        """
        inputs = self.dino_processor(images=image, return_tensors="pt").to(GRIMConfig.DEVICE)
    ...
    def interpolate_features_to_image(self, patch_tokens, spatial_shape, image_size):
        ...
        features = patch_tokens.transpose(1, 2).reshape(1, -1, spatial_shape[0], spatial_shape[1])
        ...
        return features_upsampled.squeeze(0).permute(1, 2, 0).reshape(-1, features.shape[1])
